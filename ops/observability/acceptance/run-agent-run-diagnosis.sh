...
docker run --rm \
  --name "${AGENT_RUN_DIAGNOSIS_ACCEPTANCE_CONTAINER_NAME:-agent-workbench-agent-run-diagnosis-acceptance}" \
  ...
  -e OTEL_SERVICE_NAME="${OTEL_SERVICE_NAME:-agent-workbench-api}" \
